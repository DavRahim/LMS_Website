...
export const updateAccessToken = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await redis.set(user._id, JSON.stringify(user), "EX", 604800); //7day 604800
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!oldPassword || !newPassword) {
        return next(
          new ErrorHandler("Please enter your old & new password", 400)
        );
      }
...
export const deletedUser = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.params;
